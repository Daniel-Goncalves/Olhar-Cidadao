		...
		for i in range(len(df_licitacoes)):
			...
			for i in range(len(df_licitacoes)):
    			...
    			for k in range(len(lote.index)):
        			...
        			info = read_excel('{}/{}/{}/{}'.format(path, df_licitacoes.Licitações[i], 'Lotes', arr_infos[i][k])) 
        			for j in range(len(info.index)):
            				df_materiais = df_materiais.append({'especificacoes': info.Descrição[j], 'quantidade': info.Quantidade[j], 'valor_unitario': lote.ValorUnitário[k], 'item': info.Item[j], 'fornecedor': lote.Nome_Fantasia[k], 'unidade': None, 'filename':None}, ignore_index=True)
